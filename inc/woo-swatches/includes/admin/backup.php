...
class Backup {
	 ...
	 * @static
	 *
	 * @var Softim\VariationSwatches\Admin\Backup
	 */
	 ...
	 * @static
	 *
	 * @return Softim\VariationSwatches\Admin\Backup An instance of the class.
	 */
	...
	public function __construct() {
		...
		add_filter( 'woocommerce_product_export_product_column_softim_attributes_type', [ $this, 'add_attributes_type_export_data' ], 10, 2 );
		add_filter( 'woocommerce_product_export_product_column_softim_variaiton_swatches', [ $this, 'add_variation_swatches_export_data' ], 10, 2 );
	...
	public function restore_attributes_backup_notice() {
		...
		if ( $backup && ! get_option( 'softim_variation_swatches_ignore_restore' ) ) {
			$backup_date = date( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ), $backup['time'] );
			?>
			<div class="notice notice-warning is-dismissible">
				<p>
					<?php printf( esc_html__( 'Found a backup of your attributes type. This backup was generated at %s.', 'softim' ), $backup_date ); ?>
				</p>
				<p>
					<a href="<?php echo esc_url( add_query_arg( [ 'action' => 'softim_variation_swatches_restore_backup', '_wpnonce' => wp_create_nonce( 'softim_variation_swatches_restore_backup' ) ] ) ) ?>"><?php esc_html_e( 'Restore attributes', 'softim' ) ?></a> |
					<a href="<?php echo esc_url( add_query_arg( [ 'action' => 'softim_variation_swatches_ignore_backup', '_wpnonce' => wp_create_nonce( 'softim_variation_swatches_ignore_backup' ) ] ) ) ?>"><?php esc_html_e( 'Ignore', 'softim' ) ?></a>
				</p>
			</div>
			<?php
		}

		if ( ! $backup && isset( $_GET['message'] ) && 'softim_variation_swatches_restored_backup' == $_GET['message'] ) {
			?>
			<div class="notice notice-success is-dismissible">
				<p><?php esc_html_e( 'All attribute types have been restored.', 'softim' ) ?></p>
			</div>
			<?php
		}
	}
	...
	public function handle_restore_request() {
		if ( ! isset( $_GET['action'] ) || ! isset( $_GET['_wpnonce'] ) || 'softim_variation_swatches_restore_backup' != $_GET['action'] ) {
			return;
		}
		...
		if ( $success ) {
			...
			$url = add_query_arg( [ 'message' => 'softim_variation_swatches_restored_backup' ], $url );
	...
	public function handle_ignore_request() {
		if ( ! isset( $_GET['action'] ) || ! isset( $_GET['_wpnonce'] ) || 'softim_variation_swatches_ignore_backup' != $_GET['action'] ) {
			return;
		}
	...
	public function restore() {
		...
		delete_transient( 'softim_variation_swatches_attributes' );
		delete_transient( 'tawcvs_attribute_taxonomies' );
		delete_transient( 'wc_attribute_taxonomies' );
		delete_option( 'softim_variation_swatches_ignore_restore' );
	...
	public function ignore() {
		return update_option( 'softim_variation_swatches_ignore_restore', time(), false );
	}
	...
	public static function backup() {
		...
		if ( ! empty( $backup['attributes'] ) ) {
			set_transient( 'softim_variation_swatches_attributes', $backup );
			delete_transient( 'wc_attribute_taxonomies' );
	...
	protected function get_backup_attributes() {
		$backup = get_transient( 'softim_variation_swatches_attributes' );
	...
	public function add_import_export_columns( $columns ) {
		$columns['softim_variaiton_swatches'] = esc_html__( 'Variation Swatches', 'softim' );
		$columns['softim_attributes_type']    = esc_html__( 'Attributes Swatches', 'softim' );
	...
	public function add_column_to_mapping_screen( $columns ) {
		$columns[ esc_html__( 'Variation Swatches', 'softim' ) ]  = 'softim_variaiton_swatches';
		$columns[ esc_html__( 'Attributes Swatches', 'softim' ) ] = 'softim_attributes_type';
	...
	public function process_import( $product, $data ) {
		if ( empty( $data['softim_variaiton_swatches'] ) ) {
			return $product;
		}

		$swatches_meta = (array) json_decode( $data['softim_variaiton_swatches'], true );
		...
		$product->update_meta_data( 'softim_variaiton_swatches', $swatches_meta );
	...
	public function import_attributes_type( $product, $data ) {
		if ( empty( $data['softim_attribute_type'] ) ) {
			return;
		}

		$raw_data             = (array) json_decode( $data['softim_attribute_type'], true );
	...
	public function get_attachment_id_from_url( $url, $product_id ) {
		...
		if ( ! $id && stristr( $url, '://' ) ) {
			...
			if ( ! wp_attachment_is_image( $id ) ) {
				...
				throw new \Exception( sprintf( esc_html__( 'Not able to attach "%s".', 'softim' ), $url ), 400 );
			}
		...
		if ( ! $id ) {
			/* translators: %s: image URL */
			throw new \Exception( sprintf( esc_html__( 'Unable to use image "%s".', 'softim' ), $url ), 400 );
		}
