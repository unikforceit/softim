<?php
namespace Softim\VariationSwatches\Admin;

defined( 'ABSPATH' ) || exit;

use Softim\VariationSwatches\Helper;
use Softim\VariationSwatches\Plugin;

class Product_Data {
	const META_NAME = 'softim_variaiton_swatches';
	 ...
	 * @static
	 *
	 * @var Softim\VariationSwatches\Admin\Product_Data
	 */
	 ...
	 * @static
	 *
	 * @return Softim\VariationSwatches\Admin\Product_Data An instance of the class.
	 */
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_softim_variation_swatches_add_term', [ $this, 'ajax_add_new_attribute_term' ] );
	...
	public function product_option_terms( $taxonomy, $index, $attribute ) {
		...
		<select multiple="multiple" data-placeholder="<?php esc_attr_e( 'Select terms', 'softim' ); ?>" class="multiselect attribute_values wc-enhanced-select" name="attribute_values[<?php echo esc_attr( $index ); ?>][]">
			<?php
			$args      = [
		...
		<button class="button plus select_all_attributes"><?php esc_html_e( 'Select all', 'softim' ); ?></button>
		<button class="button minus select_no_attributes"><?php esc_html_e( 'Select none', 'softim' ); ?></button>
		<button class="button fr plus add_new_attribute_with_swatches" data-type="<?php echo esc_attr( $taxonomy->attribute_type ) ?>"><?php esc_html_e( 'Add new', 'softim' ); ?></button>
	...
	public function swatches_tab( $tabs ) {
		$tabs['softim_variation_swatches'] = [
			'label'    => esc_html__( 'Swatches', 'softim' ),
			'target'   => 'softim_variation_swatches_data',
	...
	public function swatches_panel() {
		...
		<div id="softim_variation_swatches_data" class="panel woocommerce_options_panel wc-metaboxes-wrapper hidden">
			<div id="softim_variation_swatches_data_inner" class="softim-variation-swatches-product-data  wc-metaboxes">
				<?php
				...
				foreach ( $attributes as $attribute ) {
					...
					<div data-taxonomy="<?php echo esc_attr( $attribute->get_taxonomy() ); ?>" class="wc-metabox closed" rel="<?php echo esc_attr( $attribute->get_position() ); ?>">
						<h3>
							<div class="handlediv" title="<?php esc_attr_e( 'Click to toggle', 'softim' ); ?>"></div>
						...
						<div class="wc-metabox-content hidden">
							<div class="options_group">
								...
								woocommerce_wp_select( [
									'id'            => 'softim_variation_swatches[' . $attribute_name . '][type]',
									'class'         => 'select',
									'wrapper_class' => 'softim-variaton-swatches__type-field',
									'label'         => esc_html__( 'Type', 'softim' ),
									'description'   => sprintf( esc_html__( 'The default type is: %s', 'softim' ), $types[ $attribute_type ] ),
									'value'         => $attribute_swatches['type'],
									'options'       => array_merge( [ '' => esc_html__( 'Default', 'softim' ) ], $types ),
								] );
								?>
							</div>
							...
							<div class="options_group">
								<?php
								woocommerce_wp_select( [
									'id'            => 'softim_variation_swatches[' . $attribute_name . '][shape]',
									'class'         => 'select',
									'wrapper_class' => 'softim-variaton-swatches__shape-field',
									'label'         => esc_html__( 'Shape', 'softim' ),
									'description'   => sprintf( esc_html__( 'The default shape is: %s', 'softim' ), $shapes[ $default_settings['shape'] ] ),
									'value'         => $attribute_swatches['shape'],
									'options'       => array_merge( [ '' => esc_html__( 'Default', 'softim' ) ], $shapes ),
								] );
								?>
							</div>
							...
							<div class="options_group">
								<?php
								woocommerce_wp_select( [
									'id'            => 'softim_variation_swatches[' . $attribute_name . '][size]',
									'class'         => 'select',
									'wrapper_class' => 'softim-variaton-swatches__size-field',
									'label'         => esc_html__( 'Size', 'softim' ),
									'value'         => $attribute_swatches['size'],
									'options'       => [
										''       => esc_html__( 'Default', 'softim' ),
										'custom' => esc_html__( 'Custom', 'softim' ),
									],
								] );
								?>
								<p class="form-field form-field--custom-size dimensions_field <?php echo 'custom' != $attribute_swatches['size'] ? 'hidden' : '' ?>">
									<span class="wrap">
										<input type="text" name="softim_variation_swatches[<?php echo esc_attr( $attribute_name ) ?>][custom_size][width]" value="<?php echo esc_attr( $attribute_swatches['custom_size']['width'] ) ?>" size="5" placeholder="<?php esc_attr_e( 'Width', 'softim' ) ?>">
										<input type="text" name="softim_variation_swatches[<?php echo esc_attr( $attribute_name ) ?>][custom_size][height]" value="<?php echo esc_attr( $attribute_swatches['custom_size']['height'] ) ?>" size="5" placeholder="<?php esc_attr_e( 'Height', 'softim' ) ?>">
									</span>
								</p>
							</div>
	...
	protected function swatches_metabox( $args ) {
		...
		foreach ( $options as $option ) {
			...
			switch ( $args['type'] ) {
				case 'color':
					printf(
						'<span class="softim-variation-swatches__field-color">
							<span class="label">%s</span><br>
							<input type="text" name="%s" value="%s">
						</span>',
						esc_html( $label ),
						esc_attr( "softim_variation_swatches[$attribute_name][swatches][$name][color]" ),
				...
				case 'image':
					...
					printf(
						'<span class="softim-variation-swatches__field-image">
						...
						esc_html( $label ),
						esc_html__( 'Edit', 'softim' ),
						empty( $values['image'] ) ? 'hidden' : '',
						esc_html__( 'Delete Image', 'softim' ),
						esc_html__( 'Remove', 'softim' ),
						esc_attr( "softim_variation_swatches[$attribute_name][swatches][$name][image]" ),
				...
				case 'label':
					printf(
						'<span class="softim-variation-swatches__field-label">
							<input type="text" name="%s" value="%s"><br>
							<span class="label">%s</span>
						</span>',
						esc_attr( "softim_variation_swatches[$attribute_name][swatches][$name][label]" ),
	...
	public function process_product_swatches_meta( $post_id ) {
		if ( ! isset( $_POST['softim_variation_swatches'] ) || ! is_array( $_POST['softim_variation_swatches'] ) ) {
			return;
		}
		...
		foreach ( $_POST['softim_variation_swatches'] as $attribute_slug => $settings ) {
			$data[ $attribute_slug ] = [
	...
	public function ajax_add_new_attribute_term() {
		if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( $_POST['_wpnonce'], 'softim_variation_swatches_add_term' ) ) {
			wp_send_json_error( esc_html__( 'Wrong request', 'softim' ) );
		}

		if ( empty( $_POST['attribute_name'] ) || empty( $_POST['attribute_taxonomy'] ) ) {
			wp_send_json_error( esc_html__( 'Not enough data', 'softim' ) );
		}
		...
		if ( ! taxonomy_exists( $taxonomy ) ) {
			wp_send_json_error( esc_html__( 'Taxonomy is not exists', 'softim' ) );
		}

		if ( term_exists( $name, $taxonomy ) ) {
			wp_send_json_error( esc_html__( 'This term is already exists', 'softim' ) );
		}
		...
		if ( ! is_wp_error( $term ) ) {
			wp_send_json_success( [
				'message' => esc_html__( 'Added successfully', 'softim' ),
	...
	public function dialog_new_term() {
		...
		<div id="softim-variation-swatches-new-term-dialog" class="softim-variation-swatches-dialog hidden" style="display: none">
			<div class="softim-variation-swatches-modal wp-core-ui" tabindex="0" role="dialog">
				<button type="button" class="media-modal-close">
					<span class="media-modal-icon">
						<span class="screen-reader-text"><?php esc_html_e( 'Close dialog', 'softim' ) ?></span>
					</span>
				</button>
				<div class="softim-variation-swatches-modal__header"><h2><?php esc_html_e( 'Add New Term', 'softim' ) ?></h2></div>
				<div class="softim-variation-swatches-modal__content">
					<p class="form-field">
						<label>
							<?php esc_html_e( 'Name', 'softim' ) ?><br>
					...
					<p class="form-field form-field__swatches">
						<span class="softim-variation-swatches__field-color">
							<span class="label"><?php esc_html_e( 'Color', 'softim' ) ?></span><br>
							<input type="text" name="color" value="">
						</span>
						<span class="softim-variation-swatches__field-image">
							<span class="label"><?php esc_html_e( 'Image', 'softim' ) ?></span><br>
							<img src="<?php echo esc_url( $default_image ) ?>" data-placeholder="<?php echo esc_url( $default_image ) ?>" width="60" height="60">
							<a href="#" class="button-add-image"><?php esc_html_e( 'Upload', 'softim' ) ?></a>
							<a href="#" class="button-link button-remove-image delete hidden"><?php esc_html_e( 'Delete', 'softim' ) ?></a>
							<input type="hidden" name="image" value="">
						</span>
						<span class="softim-variation-swatches__field-label">
							<span class="label"><?php esc_html_e( 'Label', 'softim' ) ?></span><br>
					...
					<?php wp_nonce_field( 'softim_variation_swatches_add_term', '_wpnonce', false ) ?>
				</div>
				<div class="softim-variation-swatches-modal__footer">
					<button type="button" class="button-add button button-primary"><?php esc_html_e( 'Add New', 'softim' ) ?></button>
					<span class="softim-variation-swatches-modal__spinner spinner"></span>
					<span class="softim-variation-swatches-modal__message hidden"></span>
				</div>
			</div>
