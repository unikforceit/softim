...
class Settings {
	const OPTION_NAME = 'softim_variaiton_swatches';
	 ...
	 * @static
	 *
	 * @return Softim\VariationSwatches\Admin\Settings An instance of the class.
	 */
	...
	public function get_shape_options() {
		$options = apply_filters( 'softim_variation_swatches_shape_options', [
			'round'   => esc_html__( 'Round', 'softim' ),
			'rounded' => esc_html__( 'Rounded', 'softim' ),
			'square'  => esc_html__( 'Square', 'softim' ),
		] );
		...
		if ( current_theme_supports( 'woocommerce' ) ) {
			...
			if ( $theme_style ) {
				$options = array_merge( [ 'default' => esc_html__( 'Theme Default', 'softim' ) ], $options );
			}
		}
