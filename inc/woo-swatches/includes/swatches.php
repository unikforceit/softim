 ...
 */
namespace Softim\VariationSwatches;

defined( 'ABSPATH' ) || exit;

use Softim\VariationSwatches\Admin\Term_Meta;

class Swatches {
	/**
	 * The single instance of the class
	 *
	 * @var Softim\VariationSwatches\Swatches
	 */
	 ...
	 * Main instance
	 *
	 * @return Softim\VariationSwatches\Swatches
	 */
	...
	public function __construct() {
		...
		add_action( 'softim_swatches_grid_action', array( $this, 'product_attribute' ), 10 );
	}
	...
	public function enqueue_scripts() {
		wp_enqueue_style( 'softim', plugins_url( 'assets/css/frontend.css', SOFTIM_VARIATION_SWATCHES_FILE ), [], '20210628' );
		wp_add_inline_style( 'softim', $this->inline_style() );

		wp_enqueue_script( 'softim', plugins_url( 'assets/js/frontend.js', SOFTIM_VARIATION_SWATCHES_FILE ), [ 'jquery' ], '20210908', true );
		wp_localize_script( 'softim', 'softim_variation_swatches_params', [
	...
	public function inline_style() {
		...
		$css = '.softim-variation-swatches__item { width: ' . absint( $size['width'] ) . 'px; height: ' . absint( $size['height'] ) . 'px; line-height: ' . absint( $size['height'] ) . 'px; }';
		$css .= '.softim-variation-swatches--round.softim-variation-swatches--button .softim-variation-swatches__item {border-radius: ' . ( absint( $size['height'] ) / 2 ) . 'px}';

		return apply_filters( 'softim_variation_swatches_css', $css );
	}
	...
	public function swatches_html( $html, $args ) {
		...
		if ( ! empty( $swatches_html ) ) {
			$classes       = [
				'softim',
				'softim-variation-swatches--' . $swatches_args['swatches_type'],
				'softim-variation-swatches--' . $swatches_args['swatches_shape']
			];

			if ( $swatches_args['swatches_tooltip'] ) {
				$classes[] = 'softim-variation-swatches--has-tooltip';
			}

			$swatches_html = '<ul class="softim-variation-swatches__wrapper" data-attribute_name="' . esc_attr( $name ) . '">' . $swatches_html . '</ul>';
...
public function get_variation_attributes( $child_id, $attribute ) {
	...
	public function get_term_swatches( $term, $args ) {
		...
		switch ( $type ) {
			case 'color':
				$html = sprintf(
					'<li class="softim-variation-swatches__item softim-variation-swatches__item-%s %s" style="%s" aria-label="%s" data-value="%s" tabindex="0">
						<span class="softim-variation-swatches__name" style="background-color: %s">%s</span>
					</li>',
					esc_attr( $value ),
					$selected ? 'selected' : '',
					esc_attr( $size ),
			...
			case 'image':
				...
				$html = sprintf(
					'<li class="softim-variation-swatches__item softim-variation-swatches__item-%s %s" style="%s" aria-label="%s" data-value="%s" tabindex="0">
						<img src="%s" alt="%s">
						<span class="softim-variation-swatches__name">%s</span>
					</li>',
					esc_attr( $value ),
					$selected ? 'selected' : '',
					esc_attr( $size ),
			...
			default:
				$html = apply_filters( 'softim_variation_swatches_' . $type . '_html', '', $args );
				break;
		}
