<?php
namespace Softim\VariationSwatches\Customize;
...
class Customizer {
	 ...
	 * The setting API
	 *
	 * @var \Softim\VariationSwatches\Admin\Settings
	 */
	public $setting_api;
	...
	public function __construct() {
		$this->settings_api = \Softim\VariationSwatches\Admin\Settings::instance();
	...
	public function add_sections( $wp_customize ) {
		$section_name = 'softim_variation_swatches';

		$wp_customize->add_section(
			$section_name,
			[
				'title'    => esc_html__( 'Variation Swatches', 'softim' ),
				'description' => esc_html__( 'Some of these options can be overidden in product data settings.', 'softim' ),
				'priority' => 30,
				'panel'    => 'woocommerce',
			]
		);
		...
		$wp_customize->add_control(
			new Size_Control(
				...
				$this->settings_api->get_option_name( 'size' ),
				[
					'label'       => esc_html__( 'Swatches Size', 'softim' ),
		...
		$wp_customize->add_control(
			$this->settings_api->get_option_name( 'tooltip' ),
			[
				'label'    => esc_html__( 'Enable tooltip', 'softim' ),
				'section'  => $section_name,
				'type'     => 'checkbox',
			]
		);
		...
		$wp_customize->add_control(
			$this->settings_api->get_option_name( 'auto_button' ),
			[
				'label'    => esc_html__( 'Convert default dropdowns to buttons', 'softim' ),
				'section'  => $section_name,
				'type'     => 'checkbox',
			]
		);
		...
		$wp_customize->add_control(
			$this->settings_api->get_option_name( 'show_selected_label' ),
			[
				'label'    => esc_html__( "Show the selected attribute's label", 'softim' ),
				'section'  => $section_name,
				'type'     => 'checkbox',
			]
		);
	...
	public function add_styles() {
		?>
		<style type="text/css">
			.customize-control-softim-variation-swatches-size .customize-control-inside input[type="text"] {
				width: auto;
	...
	public function enqueue_scripts() {
		wp_enqueue_script( 'softim-variation-swatches-customizer', plugins_url( 'assets/js/customizer.js', SOFTIM_VARIATION_SWATCHES_FILE ), [ 'jquery', 'customize-base' ], '20210722', true );
	}
}
