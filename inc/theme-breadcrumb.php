...
class Softim_Breadcrumb {
	/**
	 * Array of bread Softim_Breadcrumb
	 *
	 * @var array
	 */
	...
	public function __construct( array $options = [] ) {
		...
		$this->defaults['home'] = esc_html__('Home','softim');
		$this->defaults['blog'] = esc_html__('Blog','softim');
	...
	public function theBreadcrumbs() {
		...
		foreach ( $breadcrumbs as $crumb ) {
			...
			if ( $crumb['url'] ) {
                ...
                $html .= sprintf( '<span>%s</span>', wp_kses($crumb['title'],softim()->kses_allowed_html('all')) );
			}
		...
		echo wp_kses($html,softim()->kses_allowed_html('all'));
	}
	...
	public function getBreadcrumbs() {
		...
		$breadcrumbs = apply_filters( 'softim_get_breadcrumbs', $breadcrumbs );
 ...
 * Template Functions.
 */
if ( ! function_exists( 'softim_breadcrumb' ) ) {
	/**
	 ...
	 * @package softim
	 */
	function softim_breadcrumb( $options = [] ) {
		$Crumbs = new Softim_Breadcrumb( $options );
		$Crumbs->theBreadcrumbs();
	}
}

if ( ! function_exists( 'softim_get_breadcrumbs' ) ) {
	/**
	 ...
	 * @return array
	 */
	function softim_get_breadcrumbs( $options = [] ) {
		$Crumbs = new Softim_Breadcrumb( $options );
 ...
 * Utility functions
 */
if ( ! function_exists( 'softim_breadcrumb_array_insert' ) ) {
	/**
	 ...
	 * @return array
	 */
	function softim_breadcrumb_array_insert( array $array, $element, int $position ) {
